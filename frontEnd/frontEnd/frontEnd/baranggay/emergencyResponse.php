<?php
// filepath: c:\Users\mnava\Desktop\frontEnd\frontEnd\frontEnd\baranggay\emergencyResponse.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['emergency_id'])) {
    $action = $_POST['action'];
    $emergency_id = $_POST['emergency_id'];

    // Update the status in the database
    $status = $action === 'approve' ? 'Approved' : 'Rejected';
    $update_query = "UPDATE emergency_reports SET status = ? WHERE emergency_id = ?";
    $stmt = $con->prepare($update_query);
    $stmt->bind_param("si", $status, $emergency_id);
    $stmt->execute();

    // Redirect to avoid form resubmission
    header("Location: emergencyResponse.php");
    exit();
}
...
$sql = "SELECT emergency_id, CONCAT(first_name, ' ', last_name) AS resident_name, 
        type_of_emergency AS emergency_type, location, report_date, status, description, file_path 
        FROM emergency_reports 
        WHERE (status = ? OR ? = '') 
          AND (CONCAT(first_name, ' ', last_name) LIKE ? OR type_of_emergency LIKE ? OR location LIKE ?)
        ORDER BY report_date DESC";
$stmt = $con->prepare($sql);
$search_param = '%' . $search . '%';
$stmt->bind_param("sssss", $filter, $filter, $search_param, $search_param, $search_param);
$stmt->execute();
$result = $stmt->get_result();
...
<head>
    ...
    <title>Bayanihan Hub - Emergency Response Coordination</title>
    ...
    <div class="mt-2 p-3">
        <div class="container">
            ...
            <div class="d-flex justify-content-between mb-3">
                ...
                <form method="GET" action="emergencyResponse.php" class="d-flex">
                    <!-- Filter Dropdown -->
                ...
                <table class="table table-bordered">
                    <thead class="table-primary">
                        <tr>
                            ...
                            <th>Date</th>
                            <th>Status</th>
                            <!-- <th>Photo</th> balik ko pag okay na photo path-->
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if (count($emergencies) > 0): ?>
                            <?php foreach ($emergencies as $emergency): ?>
                                <tr>
                                    <td><?= htmlspecialchars($emergency['emergency_id']) ?></td>
                                    <td><?= htmlspecialchars($emergency['resident_name']) ?></td>
                                    <td><?= htmlspecialchars($emergency['emergency_type']) ?></td>
                                    <td><?= htmlspecialchars($emergency['location']) ?></td>
                                    <td><?= htmlspecialchars(date("M d, Y", strtotime($emergency['report_date']))) ?></td>
                                    <td><?= htmlspecialchars($emergency['status']) ?></td>
                                    <!-- <td class="text-center">
                                        <?php if (!empty($emergency['photo_path'])): ?>
                                            <img src="<?= htmlspecialchars($emergency['photo_path']) ?>" 
                                                 alt="Emergency Photo" 
                                                 class="img-thumbnail" 
                                                 style="width: 70px; height: 70px; object-fit: cover;" 
                                                 data-bs-toggle="modal" 
                                                 data-bs-target="#photoModal<?= $emergency['emergency_id'] ?>"
                                    ...
                                    <td>
                                        <!-- View Button -->
                                        <button class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#viewModal<?= $emergency['emergency_id'] ?>">View</button>
                                        <!-- Approve Button -->
                                        <form method="POST" action="emergencyResponse.php" style="display: inline;">
                                            <input type="hidden" name="emergency_id" value="<?= $emergency['emergency_id'] ?>">
                                ...
                                <div class="modal fade" id="viewModal<?= $emergency['emergency_id'] ?>" tabindex="-1" aria-labelledby="viewModalLabel<?= $emergency['emergency_id'] ?>" aria-hidden="true">
                                    <div class="modal-dialog modal-lg">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="viewModalLabel<?= $emergency['emergency_id'] ?>">Emergency Details</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <div class="row">
                                                    <div class="col-md-6">
                                                        <p><strong>Resident Name:</strong> <?= htmlspecialchars($emergency['resident_name']) ?></p>
                                                        <p><strong>Emergency Type:</strong> <?= htmlspecialchars($emergency['emergency_type']) ?></p>
                                                        <p><strong>Location:</strong> <?= htmlspecialchars($emergency['location']) ?></p>
                                                        <p><strong>Date:</strong> <?= htmlspecialchars(date("M d, Y", strtotime($emergency['report_date']))) ?></p>
                                                    ...
                                                    <!-- <div class="col-md-6">
                                                        <?php if (!empty($emergency['photo_path'])): ?>
                                                            <h6 class="mb-3">Emergency Photo:</h6>
                                                            <img src="<?= htmlspecialchars($emergency['photo_path']) ?>" 
                                                                alt="Emergency Photo" 
                                                        ...
                                                        <?php else: ?>
                                                            <div class="alert alert-info">
                                                                No photo was provided for this emergency report.
                                                            </div>
                                                        <?php endif; ?>
                                                    </div> photo path -->
                                                </div>
                                ...
                                <!-- Modal for Photo Full View -->
                                <div class="modal fade" id="photoModal<?= $emergency['emergency_id'] ?>" tabindex="-1" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered modal-lg">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title">Emergency Photo</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body text-center">
                                                <img src="<?= htmlspecialchars($emergency['photo_path']) ?>" 
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="8" class="text-center">No emergency reports found.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
